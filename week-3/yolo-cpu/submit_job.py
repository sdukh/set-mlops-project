...
def prepare_job_files():
    ...
    files_to_upload = ["train_yolo.py", "config.yaml", "requirements.txt", "ray_job.py"]
...
def run_ray_job(file_contents):
    ...
    try:
        result = subprocess.run(
            [sys.executable, "ray_job.py"], capture_output=True, text=True, check=True
        )
        print(result.stdout)
        return True
...
def main():
    ...
    try:
        ...
        config = load_config()
        base_run_name = (
            config.get("run_name", "yolo-ray-training")
            if config
            else "yolo-ray-training"
        )
